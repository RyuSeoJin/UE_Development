...
**작성 원칙(필수)**: 생성하는 모든 문서는 초보자도 혼자 따라 할 수 있도록 용어 풀이·구체 절차(메뉴/노드/경로/클래스 이름)·전제와 결과·근거를 빠짐없이 담는다. "안다고 가정하고 건너뛰기" 금지. **예제 코드(.h/.cpp)에는 의미 있는 모든 줄/블록에 한국어 설명 주석을 꼼꼼하게 단다** — 매크로·지정자의 의미, 엔진 호출 시점, 값 범위·단위, 순서 로직은 단계 번호. '무엇'보다 '왜' 중심(코드 낭독형 주석 금지), 모든 코드 탭 동일 적용. 상세 기준은 플레이북 [작성 원칙].
